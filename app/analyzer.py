...
def analyze_ticket(ticket):
    ...
    # -----------------------------
    case_type = detect_case_type(ticket.complaint)
    ...
    amount = extract_amount(ticket.complaint)
    ...
    if case_type in ["wrong_transfer", "duplicate_payment", "refund_request"]:
        severity = "high"
        human_review = True
    ...
    result["case_type"] = result.get("case_type", case_type)
    result["severity"] = result.get("severity", severity)
    result["department"] = result.get("department", get_department(case_type))
    result["evidence_verdict"] = verdict
    result["human_review_required"] = human_review

    result.setdefault("reason_codes", [case_type, verdict])
